...
function ViewPhone({ brandId }) {
  ...
  useEffect(() => {
   ...
   setLoading(true);
      try {
         let url = `http://localhost:8080/product/products/category/1?page=${page}&size=${pageSize}`;
            if (brandId) {
          url = `http://localhost:8080/product?brandId=${brandId}&categoryId=1&page=${page}&size=${pageSize}`;
           }
