...
function ViewWatch({ brandId }) {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            setLoading(true);
            try {
                let url = `http://localhost:8080/product/products/category/3?page=${page}&size=${pageSize}`;
                if (brandId) {
                    url = `http://localhost:8080/product?brandId=${brandId}&categoryId=3&page=${page}&size=${pageSize}`;
                }
