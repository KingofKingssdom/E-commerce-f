...
function ViewTv({ brandId }) {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            setLoading(true);
            try {
                let url = `http://localhost:8080/product/products/category/7?page=${page}&size=${pageSize}`;
                if (brandId) {
                    url = `http://localhost:8080/product?brandId=${brandId}&categoryId=7&page=${page}&size=${pageSize}`;
                }
                ...
                setProducts(data.content); // Đảm bảo data.content là mảng
